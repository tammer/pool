...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		# delete pick where week=15 and game=5 or 14
		for user in User.objects.all():
			p = Pick.objects.get(player=user, week_number=15,game_number=5)
			p.delete()
			p = Pick.objects.get(player=user, week_number=15,game_number=14)
			p.delete()			
		# delete game where week=15 and game=5 or 14
		game = Game.objects.get(week_number=15,game_number=5)
		game.delete()
		game = Game.objects.get(week_number=15,game_number=14)
		game.delete()

	# def XXXXXXX(self, *args, **options):
